...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Edit Room Type</h1>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.room-types.update', $roomType) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                          placeholder="Room type description...">{{ old('description', $roomType->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="capacity_children" min="0"
                       value="{{ old('capacity_children', $roomType->capacity_children) }}"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
            ...
            <div>
                ...
                @if($roomType->display_image_url)
                    <img src="{{ $roomType->display_image_url }}"
                         alt="{{ $roomType->name }}"
                         class="mt-2 mb-3 h-32 w-48 object-cover rounded-md border">
                @endif
                ...
                <p class="mt-1 text-xs text-gray-500">Upload a new image only if you want to replace the current one. Max 2 MB.</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center">
                ...
                <input type="checkbox" name="is_active" value="1"
                       @checked(old('is_active', $roomType->is_active))
